        ...
        .then((user) => {
            if (!user) {
                res
                    ...
                    .json({
                        success: false,
                        message: "Por favor verifique o seu mail e password",
                    });
            } else {
                bcrypt.compare(password, user.password, (err, isMatch) => {
                    ...
                    } else if (isMatch && user.aprovado !== "Aprovado") {
                        res
                            .status(401)
                            .json({ success: false, message: "Conta aguardando aprovação" });
                    } else {
                        res
                            .status(401)
    ...
    User.findOne({ email: req.body.email })
        .then((user) => {
            if (!user) {
                res.status(401).json({
                    success: false,
                    message: "Utilizador não encontrado, porfavor verifique o seu mail e password",
                });
            }
        ...
        .then((user) => {
            if (!user) {
                res
                    ...
                    .json({
                        success: false,
                        message: "Utilizador não encontrado, porfavor verifique o seu email",
                    });
            } else {
                bcrypt.genSalt(10, (err, salt) =>
                    bcrypt.hash(req.body.password, salt, (err, hash) => {
